...
st.set_page_config(page_title="HackAttack: The Noisy Checkers", page_icon="🔇")

with open("./assets/style.css") as f:
    css = f.read()

st.markdown(f"<style>{css}</style>", unsafe_allow_html=True)
st.image("./assets/42nana_header.png", width=400, clamp=False, use_column_width="auto")
...
def create_map(restaurants, noise_data, selected_day, selected_hour):
    ...
    for restaurant in restaurants:
        noise = get_rest_noise(
            noise_data, selected_day, selected_hour, restaurant["id"]
        )
        folium.Marker(
